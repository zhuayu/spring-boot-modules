...
CREATE TABLE `rabc_permissions` (
  ...
  `group_id` bigint unsigned DEFAULT NULL COMMENT '组 ID',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `rabc_permissions_name_unique` (`name`),
  CONSTRAINT `rabc_permissions_group_id_foreign` FOREIGN KEY (`group_id`) REFERENCES `rabc_permission_groups` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
